...
def rot13(text):
    ...
    for i in text:
        if ord(i) > 64 and ord(i) < 78:
            new_text += chr(ord(i)+13)
        elif ord(i) >= 78 and ord(i) <= 91:
            new_text += chr(ord(i)-13)
        elif ord(i) > 96 and ord(i) < 110:
            new_text += chr(ord(i)+13)
...
def greekify(text):
    ...
    new_text = ""
    for i in text:
        if latin_alphabet.find(i) != -1:
            new_text += greek_alphabet[latin_alphabet.find(i)]
        elif greek_alphabet.find(i) != -1:
            new_text += latin_alphabet[greek_alphabet.find(i)]
        else:
            new_text += i
    return new_text
