...
def compile_headers(line):
    ...
    for i in range(len(line)):
        if line[i] == '#':
            counter+=1
    return '<h' + str(counter) + '>' + line[counter:] + '</h' + str(counter) + '>'
...
def compile_italic_star(line):
    ...
    >>> compile_italic_star('this is not \*italic!\*')
    'this is not \\\*italic!\\\*'
    >>> compile_italic_star(r'\*not italic\* *italic*')
    '\\\*not italic\\\* <i>italic</i>'
    >>> compile_italic_star('\\*not italic* *italic*')
    '\\\*not italic<i> *italic</i>'
    '''
    #get the position of the stars, and use the postiions to create the new string
    #don't try to implement everything at once, check intermediate steps
...
def compile_strikethrough(line):
    ...
    #print ("end", end_index)
    if start_index is not None and end_index is not None: 
        line = line [:start_index] + '<ins>' + line[start_index+2:end_index] + '</ins>' + line[end_index+2:]
    return line
...
def compile_bold_stars(line):
    ...
    #print ("end", end_index)
    if start_index is not None and end_index is not None: 
        line = line [:start_index] + '<b>' + line[start_index+2:end_index] + '</b>' + line[end_index+2:]
    return line
...
def compile_code_inline(line):
    ...
    if line.find('```') == -1:
        if start_index is not None and end_index is not None: 
            mid_line = line[start_index+1:end_index]
            mid_line = mid_line.replace("<", "&lt;")
            mid_line = mid_line.replace(">", "&gt;")
            line = line [:start_index] + '<code>' + mid_line + '</code>' + line[end_index+1:]
    return line
...
def compile_links(line):
    ...
    if line.find("](") == -1:
        return line
    line = line[:start_alt] + "<a href=" + '"' + line[start_link+1:end_link] + '"' + ">" +  line[start_alt+1:end_alt] + "</a>" + line[end_link+1:]
    return line
...
def compile_images(line):
    ...
    if line.find("](") == -1:
        return line
    line = line[:start_alt-1] + "<img src=" + '"' + line[start_link+1:end_link] + '"' + " alt=" + '"' + line[start_alt+1:end_alt] + '"'+ " />" + line[end_link+1:]
    return line
